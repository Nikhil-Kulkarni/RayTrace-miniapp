...
namespace Utilities 
{
     ...
     */
    template<class TYPE>
    void decompress_array( size_t N, size_t N_bytes, const unsigned char cdata[], int compression, TYPE *data[] );
    ...
    template<>
    void decompress_array<bool>( size_t N, size_t N_bytes, const unsigned char cdata[], int, bool *data[] );
        ...
        *cdata = new unsigned char[N_bytes];
        ...
        if ( N_zeros==N ) {
            ...
            *cdata = new unsigned char[N_bytes];
            ...
            // Store the data as a bit array indicating if each value is non-zero, and then a dense array of non-zeroed values
...
template <class TYPE>
void Utilities::decompress_array( size_t N, size_t N_bytes_in, const unsigned char cdata[], int method, TYPE *data[] ) 
{
    *data = new TYPE[N];
    ...
    if ( method==0 ) {
        ...
        if ( N_bytes_in==0 ) {
            ...
            // We are storing a bit array indicating the non-zeroed entries, and the non-zero values as a dense array
