...
struct RayTraceImageKokkosKernel {
    ...
    // Constructor
    RayTraceImageKokkosKernel( 
        int N_, int nx_, int ny_, int na_, int nb_, int nv_, 
        ConstDblVec x_, ConstDblVec y_, ConstDblVec a_, ConstDblVec b_,
        const double dx_, const double dy_, const double dz_, 
        const double da_, const double db_, ConstDblVec dv_,
        const RayTrace::ray_gain_struct* gain_, const RayTrace::ray_seed_struct* seed_,
        int method_, const Kokkos::View<const ray_struct*,Device> rays_, double scale_,
        Kokkos::View<double*,Device> image_, Kokkos::View<double*,Device> I_ang_ ):
        N(N_), nx(nx_), ny(ny_), na(na_), nb(nb_), nv(nv_),
        x0(x_), y0(y_), a0(a_), b0(b_),
        dx(dx_), dy(dy_), dz(dz_), da(da_), db(db_), dv0(dv_),
        gain(gain_), seed(seed_), method(method_), rays(rays_),
        scale(scale_), image(image_), I_ang(I_ang_) { }
...
template<>
const RayTrace::ray_gain_struct* copy_gain_device<Kokkos::Cuda>( int N, const RayTrace::ray_gain_struct* gain )
{
    return RayTrace::ray_gain_struct::copy_device(N,gain);
}
template<>
const RayTrace::ray_seed_struct* copy_seed_device<Kokkos::Cuda>( const RayTrace::ray_seed_struct* seed_in )
{
...
template<class Device>
void RayTraceImageKokkosLoop( int N, int nx, int ny, int na, int nb, int nv, 
    ...
    unsigned int& failure_code, std::vector<ray_struct>& failed_rays ) 
{
    ...
    Kokkos::View<const double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> x_host(x,nx);
    Kokkos::View<const double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> y_host(y,ny);
    Kokkos::View<const double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> a_host(a,na);
    Kokkos::View<const double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> b_host(b,nb);
    Kokkos::View<const double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> dv_host(dv,nv);
    Kokkos::View<const ray_struct*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> rays_host(&rays[0],rays.size());
    Kokkos::View<double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> image_host(image,nx*ny*nv);
    Kokkos::View<double*,Kokkos::LayoutRight,Kokkos::HostSpace,Kokkos::MemoryUnmanaged> I_ang_host(I_ang,na*nb);
    ...
    Kokkos::parallel_for( Kokkos::RangePolicy<Device>(0,rays.size()),
        RayTraceImageKokkosKernel<Device>(N,nx,ny,na,nb,nv,x_dev,y_dev,a_dev,b_dev,
        dx,dy,dz,da,db,dv_dev,gain,seed,method,rays_dev,scale,image_dev,I_ang_dev)
    );
