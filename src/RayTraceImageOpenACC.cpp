...
void RayTraceImageOpenAccLoop( int N, int nx, int ny, int na, int nb, int nv, 
    ...
    unsigned int& failure_code, std::vector<ray_struct>& failed_rays ) 
{
    ...
    #pragma acc data copyin(x[0:nx],y[0:ny],a[0:na],b[0:nb],dv[0:nv],rays2[0:N_rays]) \
        deviceptr(gain,seed) copyout(image[0:nx*ny*nv],I_ang[0:na*nb])
    {
