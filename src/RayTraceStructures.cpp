...
#endif
#ifdef USE_CUDA
    extern const RayTrace::ray_gain_struct* ray_gain_struct_copy_device_cuda( size_t N, const RayTrace::ray_gain_struct* arr );
    extern const RayTrace::ray_seed_struct* ray_seed_struct_copy_device_cuda( const RayTrace::ray_seed_struct& seed );
 ...
 * @param[in] device_arr_ptr_address  Device address for an ELEMENT_T * which will be set to copied-in array location.
 * @param[in] N  Number of ELEMENT_T in the array to copy-in.
 */
#ifdef USE_OPENACC
template <typename ELEMENT_T>
static void copy_in_array_and_set_devptr(const ELEMENT_T* array_host, ELEMENT_T** device_arr_ptr_address, size_t N)
{
    if(array_host && N>0) {
...
inline bool approx_equal( size_t N, const double *x, const double *y, double tol=1e-6 )
{
    ...
    for (size_t i=0; i<N; i++)
        equal = equal && ( (2.0*fabs((x[i]-y[i])/(x[i]+y[i]))<tol) || (x[i]+y[i]==0.0) );
    return equal;
}
...
template<class TYPE> inline void pack_buffer( TYPE value, size_t& N_bytes, char *data )
{
...
template<class TYPE> inline TYPE unpack_buffer( size_t& N_bytes, const char *data )
{
    TYPE value;
...
void check_N_bytes(const byte_array_header& head, size_t N_bytes) {
    ...
    if ( N_bytes!=N_bytes_head && N_bytes_head!=0 && head.version>=2 ) {
        std::string message = "Error: Number of bytes read does not equal the number of bytes in the header\n";
        message += Utilities::stringf("   N_bytes_read = %i, N_bytes_header = %i\n",(int)N_bytes,(int)N_bytes_head);
        RAY_ERROR(message);
    }
}
...
RayTrace::plasma_struct::plasma_struct( plasma_struct&& rhs ):
    plasma_struct()
{
    swap(rhs);
}
...
void RayTrace::plasma_struct::delete_data() 
{
    ...
    delete [] R;        R  = NULL;
    delete [] u;        u  = NULL;
    delete [] du_dz;    du_dz = NULL;
    delete [] dz_rad;   dz_rad = NULL;
}
bool RayTrace::plasma_struct::valid() const
{
    bool is_valid = true;
    if ( data_type == 1 ) 
    for (int i=0; i<N; i++)   { if (  T[i]!=T[i]  ) { is_valid = false; } }
    if ( data_type==1 ) {
        for (int i=0; i<N*J; i++) { if (  x[i]!=x[i]  ) { is_valid = false; } }
    } else {
        for (int i=0; i<N*J; i++) { 
            if ( x[i]==x[i] && y[i]!=y[i] )
                is_valid = false; 
    ...
    for (int i=0; i<N*J; i++) { if ( x[i]==x[i] && Te[i]!=Te[i] ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if ( x[i]==x[i] && Ti[i]!=Ti[i] ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if ( x[i]==x[i] && Ne[i]!=Ne[i] ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if ( x[i]==x[i] && Ni[i]!=Ni[i] ) { is_valid = false; } }
    if ( data_type==1 ) {
        for (int i=0; i<N*J; i++) { if ( R[i]!=R[i] ) { is_valid = false; } }
    }
    return is_valid;
}
...
******************************************************************/
RayTrace::gain_struct::gain_struct() 
{
    memset(this,0,sizeof(gain_struct));
}
#ifdef ENABLE_MOVE_CONSTRUCTOR
RayTrace::gain_struct::gain_struct( gain_struct&& rhs ):
    gain_struct()
{
    swap(rhs);
}
...
{
    bool is_valid = true;
    for (int i=0; i<N*J; i++) { if (   g[i]!=g[i]   ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if (   E[i]!=E[i]   ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if (   W[i]!=W[i]   ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if ( dvL[i]!=dvL[i] ) { is_valid = false; } }
    for (int i=0; i<N*J; i++) { if ( dvD[i]!=dvD[i] ) { is_valid = false; } }
    return is_valid;
}
bool RayTrace::gain_struct::operator==(const RayTrace::gain_struct& rhs) const
{
    if ( N!=rhs.N || J!=rhs.J || Z_max!=rhs.Z_max || Z_l!=rhs.Z_l || nu!=rhs.nu|| nl!=rhs.nl )
        return false;
...
std::pair<char*,size_t> RayTrace::gain_struct::pack(int compression) const
{
    ...
    delete [] tmp;
    // Save g, E, dvL, dvD
    unsigned char *data_g=NULL, *data_E=NULL, *data_W=NULL, *data_dvL=NULL, *data_dvD=NULL;
    ...
    size_t Ns = 0;
    memcpy(&data_char[Ns],data_g,size_g);       Ns += size_g;
    memcpy(&data_char[Ns],data_E,size_E);       Ns += size_E;
    memcpy(&data_char[Ns],data_W,size_W);       Ns += size_W;
    memcpy(&data_char[Ns],data_dvL,size_dvL);   Ns += size_dvL;
    memcpy(&data_char[Ns],data_dvD,size_dvD);   Ns += size_dvD;
    N_bytes += Ns;
    delete [] data_g;
    delete [] data_E;
    delete [] data_W;
...
void RayTrace::gain_struct::unpack( std::pair<const char*,size_t> data_in ) {
    ...
    Utilities::decompress_array<bool>(J,N_bytes_ii,(unsigned char*)&data[N_bytes],compression,&(ii));
    N_bytes += N_bytes_ii;
    // Load g, E, dvL, dvD
    ...
    int NJ = N*J;
    Utilities::decompress_array<double>(NJ,size_g,(unsigned char*)&data[N_bytes],compression,&g);
    N_bytes += size_g;
    Utilities::decompress_array<double>(NJ,size_E,(unsigned char*)&data[N_bytes],compression,&E);
    N_bytes += size_E;
    if ( head.version >= 3 ) {
        Utilities::decompress_array<double>(NJ,size_W,(unsigned char*)&data[N_bytes],compression,&W);
        N_bytes += size_W;
    } else {
        W = new double[N*J];
        memset(W,0,N*J*sizeof(double));
    }
    Utilities::decompress_array<double>(NJ,size_dvL,(unsigned char*)&data[N_bytes],compression,&dvL);
    N_bytes += size_dvL;
    Utilities::decompress_array<double>(NJ,size_dvD,(unsigned char*)&data[N_bytes],compression,&dvD);
    N_bytes += size_dvD;
    // Check the number of bytes read
    check_N_bytes( head, N_bytes );
}
...
RayTrace::EUV_beam_struct::EUV_beam_struct():
    run_ASE(true), run_sat(true), run_refract(true), R_scale(-1), G_scale(-1),
    lambda(0), A(0), Nc(0), x(NULL), y(NULL), a(NULL), b(NULL), z(NULL),
    v(NULL), dx(0), dy(0), da(0), db(0), dz(0), dv(NULL), v0(0),
    nx(0), ny(0), nz(0), na(0), nb(0), nv(0)
{
}
RayTrace::EUV_beam_struct::~EUV_beam_struct()
{
...
#ifdef ENABLE_MOVE_CONSTRUCTOR
RayTrace::EUV_beam_struct::EUV_beam_struct( EUV_beam_struct&& rhs ):
    EUV_beam_struct()
{
    swap(rhs);
}
...
{
    bool is_valid = true;
    for (int i=0; i<nx; i++) { if ( x[i]!=x[i] ) { is_valid = false; } }
    for (int i=0; i<ny; i++) { if ( y[i]!=y[i] ) { is_valid = false; } }
    for (int i=0; i<nz; i++) { if ( z[i]!=z[i] ) { is_valid = false; } }
    for (int i=0; i<na; i++) { if ( a[i]!=a[i] ) { is_valid = false; } }
    for (int i=0; i<nb; i++) { if ( b[i]!=b[i] ) { is_valid = false; } }
    for (int i=0; i<nv; i++) { if ( v[i]!=v[i] ) { is_valid = false; } }
    for (int i=0; i<nv; i++) { if ( dv[i]!=dv[i] ) { is_valid = false; } }
    return is_valid;
}
bool RayTrace::EUV_beam_struct::operator==(const RayTrace::EUV_beam_struct& rhs) const
{
    if ( nx!=rhs.nx || ny!=rhs.ny || nz!=rhs.nz || na!=rhs.na || nb!=rhs.nb || nv!=rhs.nv )
        return false;
    ...
    if ( !approx_equal(R_scale,rhs.R_scale) || !approx_equal(G_scale,rhs.G_scale) )
        return false;
    if ( !approx_equal(lambda,rhs.lambda) || !approx_equal(A,rhs.A) || !approx_equal(Nc,rhs.Nc) )
        return false;
    if ( !approx_equal(dx,rhs.dx) || !approx_equal(dy,rhs.dy) || !approx_equal(dx,rhs.dx) || 
         !approx_equal(da,rhs.da) || !approx_equal(v0,rhs.v0) )
        return false;
    bool equal = true;
...
RayTrace::seed_beam_shape_struct::seed_beam_shape_struct(const RayTrace::seed_beam_shape_struct& rhs)
{
    T = NULL;
    It = NULL;
    Ivt = NULL;
...
RayTrace::seed_beam_shape_struct& RayTrace::seed_beam_shape_struct::operator=(const RayTrace::seed_beam_shape_struct& rhs)
{
    if ( this != &rhs ) {
...
bool RayTrace::seed_beam_shape_struct::valid() const
{
    bool is_valid = true;
    for (int i=0; i<n; i++) { if ( T[i]!=T[i] ) { is_valid = false; } }
    for (int i=0; i<3*n; i++) { if ( It[i]!=It[i] ) { is_valid = false; } }
    for (int i=0; i<3*n*nv; i++) { if ( Ivt[i]!=Ivt[i] ) { is_valid = false; } }
    return is_valid;
}
bool RayTrace::seed_beam_shape_struct::operator==(const RayTrace::seed_beam_shape_struct& rhs) const
{
...
void RayTrace::seed_beam_shape_struct::unpack( std::pair<const char*,size_t> data_in ) 
{
    ...
    if ( compression==0 ) {
        ...
        N_bytes += size_T;
        Utilities::decompress_array( 3*n, size_It, (unsigned char*) &data[N_bytes], compression, &It );
        N_bytes += size_It;
        Utilities::decompress_array( 3*n*nv, size_Ivt, (unsigned char*) &data[N_bytes], compression, &Ivt );
        N_bytes += size_Ivt;
    } else if ( compression==2 ) {
        // The data was saved without zeros and in single precision
        float *tmp_data_T, *tmp_data_It, *tmp_data_Ivt;
        Utilities::decompress_array( n, size_T, (unsigned char*) &data[N_bytes], compression, &tmp_data_T );
        N_bytes += size_T;
        Utilities::decompress_array( 3*n, size_It, (unsigned char*) &data[N_bytes], compression, &tmp_data_It );
        N_bytes += size_It;
        Utilities::decompress_array( 3*n*nv, size_Ivt, (unsigned char*) &data[N_bytes], compression, &tmp_data_Ivt );
        N_bytes += size_Ivt;
        T = new double[n];
        It = new double[3*n];
        Ivt = new double[3*n*nv];
...
bool RayTrace::seed_beam_struct::operator==(const RayTrace::seed_beam_struct& rhs) const
{
    if ( nx!=rhs.nx || ny!=rhs.ny || na!=rhs.na || nb!=rhs.nb )
        return false;
    if ( !approx_equal(dx,rhs.dx) || !approx_equal(dy,rhs.dy) || !approx_equal(da,rhs.da) || !approx_equal(db,rhs.db) )
        return false;
    if ( !approx_equal(Wx,rhs.Wx) || !approx_equal(Wy,rhs.Wy) || !approx_equal(Wa,rhs.Wa) || !approx_equal(Wb,rhs.Wb) )
        return false;
    if ( !approx_equal(Wv,rhs.Wv) || !approx_equal(Wt,rhs.Wt) )
        return false;
    if ( !approx_equal(x0,rhs.x0) || !approx_equal(y0,rhs.y0) || !approx_equal(a0,rhs.a0) || !approx_equal(b0,rhs.b0) )
        return false;
    if ( !approx_equal(t0,rhs.t0) || !approx_equal(E,rhs.E) || !approx_equal(target,rhs.target) || !approx_equal(chirp,rhs.chirp) )
        return false;
    bool equal = true;
    equal = equal && approx_equal(nx,x,rhs.x);
    equal = equal && approx_equal(ny,y,rhs.y);
...
* Function to convert a byte array to a seed_beam data structure      *
**********************************************************************/
size_t read_old_seed_shape_data( const char* data, int nv, int compression, RayTrace::seed_beam_shape_struct& seed_shape );
...
size_t read_old_seed_shape_data( const char* data, int nv, int compression, RayTrace::seed_beam_shape_struct& seed_shape )
{
    ...
    if ( compression==0 ) {
        ...
        size_t size_Ivt = data_int[2];
        N_bytes += 3*sizeof(int);
        Utilities::decompress_array( n_T, N_bytes, (unsigned char*) &data[N_bytes], compression, &T );
        N_bytes += size_T;
        Utilities::decompress_array( n_It, N_bytes, (unsigned char*) &data[N_bytes], compression, &It );
        N_bytes += size_It;
        Utilities::decompress_array( n_Ivt, N_bytes, (unsigned char*) &data[N_bytes], compression, &Ivt );
        N_bytes += size_Ivt;
    } else if ( compression==2 ) {
        // The data was saved without zeros and in single precision
        data_int = (int *) &data[N_bytes];
        ...
        N_bytes += 3*sizeof(int);
        float *tmp_data_T, *tmp_data_It, *tmp_data_Ivt;
        Utilities::decompress_array( n_T, N_bytes, (unsigned char*) &data[N_bytes], compression, &tmp_data_T );
        N_bytes += size_T;
        Utilities::decompress_array( n_It, N_bytes, (unsigned char*) &data[N_bytes], compression, &tmp_data_It );
        N_bytes += size_It;
        Utilities::decompress_array( n_Ivt, N_bytes, (unsigned char*) &data[N_bytes], compression, &tmp_data_Ivt );
        N_bytes += size_Ivt;
        T = new double[n_T];
        It = new double[n_It];
        Ivt = new double[n_Ivt];
...
bool RayTrace::ray_seed_struct::is_zero( const EUV_beam_struct& euv_beam ) const
{
    ...
    for (int i=0; i<euv_beam.nx; i++) {
        if ( euv_beam.x[i]<x[0][0] || euv_beam.x[i]>x[0][dim[0]-1] ) { continue; }
...
const RayTrace::ray_seed_struct* RayTrace::ray_seed_struct::copy_device() const
{
    ...
    ray_seed_struct* dev_instance = static_cast<ray_seed_struct*>(acc_copyin(instance, sizeof(ray_seed_struct)));
...
void RayTrace::ray_seed_struct::free_device( const ray_seed_struct* host_seed, const ray_seed_struct* device_seed )
{
    NULL_USE(host_seed);
    if ( device_seed==NULL )
        return;
	...
	ray_seed_struct* instance = const_cast<ray_seed_struct*>(host_seed); // bad API const-ness spec.
...
void RayTrace::intensity_step_struct::delete_data() 
{
    ...
    delete [] E_ang;    E_ang = NULL;
    delete [] W;        W     = NULL;
    for (int i=0; i<N_SEED_MAX; i++) {
        delete [] E_v_seed[i];      E_v_seed[i]   = NULL;
        delete [] image_seed[i];    image_seed[i] = NULL;
        delete [] E_ang_seed[i];    E_ang_seed[i] = NULL;
    }
    memset(this,0,sizeof(intensity_step_struct));
}
...
{
    if ( nx!=rhs.nx || ny!=rhs.ny || na!=rhs.na || nb!=rhs.nb || nv!=rhs.nv || N_seed!=rhs.N_seed )
...
RayTrace::intensity_struct::intensity_struct():
    E_v(NULL), image(NULL), E_ang(NULL), E_sum(NULL), I_it(NULL), E_tot(0),
    W(NULL), N_seed(0), N(0), nx(0), ny(0), na(0), nb(0), nv(0)
{
...
#ifdef ENABLE_MOVE_CONSTRUCTOR
RayTrace::intensity_struct::intensity_struct( intensity_struct&& rhs ):
    intensity_struct()
{
    swap(rhs);
}
...
void RayTrace::intensity_struct::delete_data() 
{
    ...
    delete [] image;    image = NULL;
    delete [] E_ang;    E_ang = NULL;
    delete [] E_sum;    E_sum = NULL;
    delete [] I_it;     I_it  = NULL;
    delete [] W;        W     = NULL;
    E_tot = 0;
    for (int i=0; i<N_seed; i++) {
        ...
        delete [] image_seed[i];    image_seed[i] = NULL;
        delete [] E_ang_seed[i];    E_ang_seed[i] = NULL;
        delete [] E_sum_seed[i];    E_sum_seed[i] = NULL;
        delete [] I_it_seed[i];     I_it_seed[i]  = NULL;
        E_tot_seed[i] = 0;
    }
...
void RayTrace::intensity_struct::copy_step( int i, 
    const EUV_beam_struct& euv_beam, const intensity_step_struct& I_step )
{
    // Check the inputs
    RAY_ASSERT(nx==I_step.nx&&ny==I_step.ny&&na==I_step.na&&nb==I_step.nb&&nv==I_step.nv);
...
{
    if ( N!=rhs.N || nx!=rhs.nx || ny!=rhs.ny || na!=rhs.na || nb!=rhs.nb || nv!=rhs.nv || N_seed!=rhs.N_seed )
        return false;
    bool equal = true;
...
{
    int d = 0;
    if ( a!=NULL ) { d=std::max(d,a->depth()); };
    if ( b!=NULL ) { d=std::max(d,b->depth()); };
    return d+1;
}
...
{
    memset(this,0,sizeof(ray_gain_struct));
}
...
const RayTrace::ray_gain_struct* RayTrace::ray_gain_struct::copy_device( size_t N, const ray_gain_struct* arr )
{
#if defined(USE_CUDA)
    ...
    ray_gain_struct* dev_instances = static_cast<ray_gain_struct*>(acc_copyin(instances, N*sizeof(ray_gain_struct)));
    // Had several options to deep copy the member arrays.
    // Taking the approach assuming this structure is *rarely* instantiated, modified,
    // or deleted from the device. Thus allocation on device will be most compatible with
...
void RayTrace::ray_gain_struct::free_device( size_t N, const ray_gain_struct* host_arr, const ray_gain_struct* device_arr )
{
    if ( device_arr==NULL )
        return;
#if defined(USE_CUDA)
    ...
    ray_gain_struct* instances = const_cast<ray_gain_struct*>(host_arr); // bad API const-ness spec.
...
RayTrace::apply_input_params::apply_input_params( const RayTrace::apply_input_params& rhs ):
    T0(rhs.T0), dT(rhs.dT), NN(rhs.NN), euv_beam(rhs.euv_beam), plasma(rhs.plasma),
    gain(rhs.gain), N_seed(rhs.N_seed), seed_beam(rhs.seed_beam), N_meta(rhs.N_meta),
    W0(rhs.W0), ion(rhs.ion), load_balance(rhs.load_balance), tpool(rhs.tpool), 
    interp_W_method(rhs.interp_W_method)
{
}
RayTrace::apply_input_params& RayTrace::apply_input_params::operator=( const RayTrace::apply_input_params& rhs )
{
    if ( this == &rhs )
        return *this;
    this->T0 = rhs.T0;
...
bool RayTrace::load_balance_struct::valid( int J0, int rad_type, const bool *ii ) const
{
    ...
    if ( rank!=rank0 || size!=size0 ) {
        char tmp[100];
        sprintf(tmp,"   rank = %i, size = %i, load_balance = %i, %i\n",rank0,size0,rank,size);
    ...
    if ( size>1 ) {
        ...
        if ( rad_type==4 ) {
            // Note: Parallel processing of the atomic populations is not currently supported for full radiation transport
            for (int i=0; i<J; i++) {
                if ( proc_zone[i]!=0 ) {
                    perr << "Error, full radiation transport is used, processor 0 must perform all atomic calculations\n";
                    return false;
                }
            }
        } else {
