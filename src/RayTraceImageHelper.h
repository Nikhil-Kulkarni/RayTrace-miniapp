...
#endif
HOST_DEVICE
inline void calc_seed_inline( const RayTrace::ray_seed_struct& seed,
    double x, double y, double a, double b, double *Iv ) 
{
    double f = 0.0;
    // Check to see if the point is within the grid given 
    if ( x>=seed.x[0][0] && x<=seed.x[0][seed.dim[0]-1] && 
         y>=seed.x[1][0] && y<=seed.x[1][seed.dim[1]-1] && 
         a>=seed.x[2][0] && a<=seed.x[2][seed.dim[2]-1] && 
...
#endif
HOST_DEVICE
inline float propagate( vec_struct& r, vec_struct& s, float n0,
    float dn_dx, float dn_dy, float dx[3], float c )
{
    float sum = 0.0f;
    float dz_max = c*1.00001f*dx[2];
    r.x=0;  r.y=0;  r.z=0;
    float n = n0;
    while ( fabs(r.x)<dx[0] && fabs(r.y)<dx[1] && fabs(r.z)<dx[2] && fabs(n-n0)<0.05 ) {
        // Compute some basic variables
        n = n0 + r.x*dn_dx + r.y*dn_dy;                         // n(r)
        float t = (s.x*dn_dx+s.y*dn_dy+1e-12f)/n;                // 1/n*dot(s,grad(n))
        float f[3] = {dn_dx/n-s.x*t,dn_dy/n-s.y*t,-s.z*t};      // 1/n*(grad(n)-dot(s,grad(n)))
        ...
        float step3 = c*0.05f*(fabs(s.x)+5e-4f)/(fabs(f[0])+1e-8f); // step length based on change of sx
        float step4 = c*0.05f*(fabs(s.y)+5e-4f)/(fabs(f[1])+1e-8f); // step length based on change of sy
...
#endif
HOST_DEVICE
inline float propagate2( vec_struct& pos, vec_struct& s, float dz, 
    const double x[2], const double y[2], const float range[4], 
    const double n[4], bool abs_y, float c )
{
    float z = 0.0f;
    float ds_sum = 0.0f;
    ...
    while ( pos.x>range[0] && pos.x<range[1] && y2>range[2] && y2<range[3] && z<0.999*dz ) {
        ...
        if ( abs_y && pos.y<0 )
            dn_dy = -dn_dy;
        // Propagate
        vec_struct r;   r.x=0;  r.y=0;  r.z=0;
...
inline int RayTrace_calc_ray( const ray_struct& ray, const int N, const float dz0, 
    ...
    int K, int method, double *Iv, ray_struct& ray2, 
    const float c=0.5, const double *dv=NULL, float *debug=NULL ) 
{
    ...
    for (short int i=0; i<N-1 && !escaped; i++) {
        ...
        for (short int iz=0; iz<N_SUB; iz++) {
            ...
            while ( z < 0.995f*z_stop ) {
                ...
                if ( pos.x<range[0] || pos.x>range[1] || pos.y<range[2] || pos.y>range[3] || s.z*s.z<0.01 ) {
                    escaped = true;
                    break;
                } 
                // Ray is in plasma
                ...
                float E0 = use_emis ? bilinear(dxi,dyi,ptr_E0[i1],ptr_E0[i2],ptr_E0[i3],ptr_E0[i4]):0.0f;
                // Update x and s
                pos.z = 0.0f;
        ...
        for (int i=0; i<N-1; i++) {
            for (int is=0; is<N_SUB; is++) {
                ...
                for (int k=0; k<K; k++) {
                    ...
                    if ( fabs(gl)<1e-3 ) {
                        Iv[k] = el*(1.0+0.5*gl*(1.0+0.3333333333*gl)) + Iv[k]*(1.0+gl*(1.0+0.5*gl));
                    } else {
