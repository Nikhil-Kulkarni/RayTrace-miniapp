...
namespace RayTrace {
    ...
    struct plasma_struct {
        ...
        //! operator!=
        inline bool operator!=(const plasma_struct& rhs) const { return !(this->operator==(rhs)); }
      protected:
    ...
    struct EUV_beam_struct {
        ...
        double R_scale;     //!<  Scale the self-similar width by a factor for the refraction effects (1.5D runs only)
        double G_scale;     //!<  Scale the self-similar width by a factor for the gain effects (1.5D runs only)
        ...
        //! operator!=
        inline bool operator!=(const EUV_beam_struct& rhs) const { return !(this->operator==(rhs)); }
      protected:
    ...
    struct seed_beam_shape_struct {
        ...
        int nv;             //!<  The number of points in v, dv
        double *T;          //!<  Temporal grid used to describe the temporal shape of the seed profile ( 1 x nT )
        ...
        //! operator!=
        inline bool operator!=(const seed_beam_shape_struct& rhs) const { return !(this->operator==(rhs)); }
    };
    ...
    struct seed_beam_struct {
        ...
        //! operator!=
        inline bool operator!=(const seed_beam_struct& rhs) const { return !(this->operator==(rhs)); }
      protected:
    ...
    struct plasma_gain_step_struct {
      ...
      protected:
        ...
        plasma_gain_step_struct& operator=(const plasma_gain_step_struct&);   // Private assignment operator
    };
    ...
    struct ray_gain_struct {
         ...
         */
        static void free_device( size_t N, const ray_gain_struct* host_arr, const ray_gain_struct* device_arr );
      protected:
    ...
    struct create_image_struct {
        ...
        int N_start;                    //!<  First ray to process (set to 0 for domain based decomposition, otherwise set to a unique number for each thread)
        int N_parallel;                 //!<  Number of rays processed in parallel (set to 0 for domain based decomposition, otherwise set to the number of threads)
        double dz;                      //!<  Grid size along the length 
        const EUV_beam_struct *euv_beam; //!<  Input beam structure that contains information about the desired euv beam grid 
        ...
        double *image;                  //!<  Output Spatial intensity image (Nv x Nx x Ny) (will be allocated by create_image and free'd by destructor)
        double *I_ang;                  //!<  Output Intensity in angular space  (Na x Nb)  (will be allocated by create_image and free'd by destructor)
        //! Constructor
        create_image_struct();
        //! Destructor
        ~create_image_struct();
    ...
    struct intensity_step_struct {
      ...
      private:
        ...
        intensity_step_struct& operator=(const intensity_step_struct&); // Private assignment operator
        int nx, ny, na, nb, nv;
    ...
    struct intensity_struct {
        ...
        //! operator!=
        inline bool operator!=(const intensity_struct& rhs) const { return !(this->operator==(rhs)); }
        //! Access N
        inline int get_N() const { return N; }
        //! Access nx
    ...
    struct load_balance_struct {
        ...
        int *proc_zone;                 //!<  Which processor is in charge of which zone (1xJ)
            ...
            int i_ASE[4];               //!<  Starting index for [x,y,a,b] for the ASE computation (for the current processor)
            int n_ASE[4];               //!<  Number of point in [x,y,a,b] for the ASE computation (for the current processor)
            int i_seed[4];              //!<  Starting index for [x,y,a,b] for the seed computation (for the current processor)
            int n_seed[4];              //!<  Number of point in [x,y,a,b] for the seed computation (for the current processor)
        #elif PARALLEL_METHOD==2
            int N_start;                //!<  First ray to process (set to 0 for domain based decompoisition, otherwise set to a unique number for each thread)
            int N_parallel;             //!<  Number of rays processed in parallel (set to 0 for domain based decompoisition, otherwise set to the number of threads)
        #else
            #error
        #endif
    ...
    struct apply_input_params {
        ...
        int N_seed;                         //!<  Number of seed beams
        const seed_beam_struct *seed_beam;  //!<  Array containing th seed beams seed beam structure (N_seed)
        ...
        const load_balance_struct *load_balance; //!< Structure containing the load balancing information
